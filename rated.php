...
$sessionid = $_SESSION['userProfileID'];
$sqlrated = "SELECT * FROM profilerated where userProfileID =" . $sessionid." AND Rated = 0 LIMIT 1";

$resultrated = mysqli_query($conn , $sqlrated);
if ($rowrated = mysqli_fetch_array($resultrated)){
    $House = $rowrated['HouseID'];
    


if($nextID < $MaxID){
    //css is doing difficult thats why i used the style element in the a tag
    echo " <br>
    <div class='center'>
    <button class='next'>
    <a class='texty' style='
